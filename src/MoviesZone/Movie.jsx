...
const Movie = () => {
  ...
  const filterByCategory=(cat)=>{
    setMovieList(movie.filter((data)=>data.category==cat))
  ...
  return (
    <>
      <div className="my-3"style={{width:"1000px",margin:'auto'}}>
        ...
        <button onClick={()=>setMovieList(movie)} type="button"  className="btn btn-outline-primary mx-3">
          All
        </button>
        <button onClick={()=>filterByCategory("Action")} type="button"  className="btn btn-outline-secondary mx-3">
          Action
        </button>
        <button onClick={()=>filterByCategory("Thriller")} type="button"  className="btn btn-outline-info mx-3">
          Thriller
        </button>
